...
func (rect Rectangle) draw(scn screen) (err error) {
	...
	//var lr Point = Point{rect.ur.x, rect.ll.y}
	//var ul Point = Point{rect.ll.x, rect.ur.y}
	if outOfBounds(rect.ll, scn) || outOfBounds(rect.ur, scn) { //|| outOfBounds(lr, scn) || outOfBounds(ul, scn) {
...
func (circ Circle) draw(scn screen) (err error) {
	...
	var top Point = Point{circ.center.x, circ.center.y + circ.r}
	var left Point = Point{circ.center.x - circ.r, circ.center.y}
	var right Point = Point{circ.center.x + circ.r, circ.center.y}
	var bottom Point = Point{circ.center.x, circ.center.y - circ.r}
	if outOfBounds(top, scn) || outOfBounds(left, scn) || outOfBounds(right, scn) || outOfBounds(bottom, scn) {
		return outOfBoundsErr
	}
...
func (display *Display) clearScreen() {
	...
	for i := range display.matrix {
		for j := range display.matrix[i] {
			display.matrix[i][j] = Color{255, 255, 255}
		}
	}
}
...
func (display *Display) getPixel(x, y int) (c Color, err error) {
	// TO DO: Implement this method
	if colorUnknown(display.matrix[y][x]) {
		return display.matrix[y][x], colorUnknownErr
	}
...
func (display *Display) screenShot(f string) (err error) {
	...
	fmt.Fprintln(file, display.maxX, display.maxY)
	fmt.Fprintln(file, 255)
	for i := range display.matrix {
		for j := range display.matrix[i] {
			c := display.matrix[i][j]
			fmt.Fprint(file, c.r, c.g, c.b, " ")
		}
		fmt.Fprintln(file, "")
	}
